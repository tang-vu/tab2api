...
impl TunnelManager {
    ...
    pub fn enable_access(&self) -> Result<TunnelStatus, String> {
        self.enable(false)
    }

    pub fn enable_bearer_only(&self, accepted: bool) -> Result<TunnelStatus, String> {
        if !accepted {
            return Err(
    ...
    fn enable(&self, bearer_only: bool) -> Result<TunnelStatus, String> {
        #[cfg(not(windows))]
        return Err("Cloudflare Tunnel setup is currently available only on Windows".into());
...
mod tests {
    ...
    fn bearer_only_rejects_missing_risk_acceptance_before_running_a_script() {
        ...
        assert_eq!(
            manager.enable_bearer_only(false).unwrap_err(),
