 ...
 */
export async function assertTemporaryChat(
  page: Page,
  signal: AbortSignal,
  deadlineAt?: number,
): Promise<void> {
  const attempts = boundedAttempts(deadlineAt, 12, INITIAL_STATE_POLL_MS);
...
export async function selectEffort(
  ...
  await abortRace(control.click(), signal, false);
  ...
  for (const pattern of EFFORT_LABELS[effort]) {
    ...
    try {
      await abortRace(
        option.waitFor({ state: 'visible', timeout: 1_500 }).then(() => option.click()),
        signal,
        false,
      );
      return;
      ...
      if (error instanceof AppError && error.code === 'timeout') throw error;
