...
async function doctor(): Promise<void> {
  ...
  const checks: Array<[string, () => Promise<string>]> = [
    ...
    config.browserBackend === 'playwright'
      ? [
          'Chromium executable',
          async () => {
      ...
      : [
          'GPM Login Local API/profile',
          async () => {
            const manager = new GpmBrowserManager(config);
            await manager.checkApi();
            return `${config.gpmBaseUrl} (profile configured)`;
          },
        ],
    [
  ...
  if (failed) {
    print(
      config.browserBackend === 'gpm'
        ? 'Doctor found issues. Start GPM Login, verify its profile ID/Local API URL, then run `npm run login`.'
