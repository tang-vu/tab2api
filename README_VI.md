...
Tạo một client key riêng có thể thu hồi trong tab **Key & Usage** của app desktop hoặc bằng `npm run keys -- create "Claude Code"`. Hộp thoại một lần của app có thể sao chép trọn bộ cấu hình PowerShell bên dưới kèm key mới mà không lưu lại. Chỉ cấu hình cửa sổ PowerShell hiện tại:
...
Trường tùy chọn `reference_images` gắn tối đa bốn data URL PNG/JPEG/WebP làm ảnh tham chiếu, để ảnh mới bám theo ảnh bạn đã có:

```powershell
curl.exe http://127.0.0.1:3210/v1/images/generations `
  -H "Authorization: Bearer $token" `
  -H "Content-Type: application/json" `
  -d '{"prompt":"Vẫn nhân vật đó, nhưng dưới mưa","reference_images":["data:image/png;base64,iVBORw0KGgo..."]}'
```
...
App desktop có cùng luồng trong tab **Key & Usage** khi dịch vụ cục bộ đang chạy. Native layer yêu cầu đúng identity `/healthz` không cần xác thực qua một kết nối loopback riêng trước khi đọc và gửi administrator key, sau đó giới hạn kích thước và kiểm tra chặt response có xác thực. App liệt kê metadata, tạo client key và chỉ hiện plaintext một lần, cho phép chủ động copy riêng key hoặc cấu hình PowerShell Claude Code không lưu lại, xác nhận trước khi thu hồi, xem/reset usage không chứa nội dung, đồng thời không bao giờ đưa administrator key ra WebView. Thẻ dịch vụ tách rõ sức khỏe của tiến trình local với độ sẵn sàng thật của giao diện ChatGPT; lượt kiểm tra có xác thực được giới hạn thời gian, chỉ chạy khi cần và đưa ra hướng xử lý bằng ngôn ngữ đang chọn thay vì liên tục mở tab nền. Tab **Tài liệu API** có thể xuất tài liệu canonical thành một file Markdown mới trong Downloads để làm context cho LLM; file xuất không chứa credential.
...
- Hỗ trợ text với role `system`, `developer`, `user`, `assistant`; vision nhận data URL PNG/JPEG/WebP có giới hạn, và tạo ảnh nhận đúng loại data URL đó qua `reference_images`. URL ảnh từ xa bị từ chối.
