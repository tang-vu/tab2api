...
| done   | 10. Integrated desktop browser experience | Windows app-mode Chromium is securely docked into the desktop shell, CDP remains loopback-only, manual reauthentication remains automation-free, and non-Windows behavior is explicit | Rust/Node contract tests, live login/session/API verification, packaged Windows smoke |
...
- **Integrated browser:** a native embedded-browser feasibility pass is in progress. The accepted design must keep the dedicated profile, never export browser storage, bind any DevTools transport only to loopback, and retain a fully manual no-automation reauthentication mode. Cross-process Win32 docking is Windows-specific and must have a documented companion-window fallback on macOS/Linux.
...
- Playwright CDP attachment and fidelity warning: https://playwright.dev/docs/api/class-browsertype#browser-type-connect-over-cdp
- Chrome 136+ remote-debugging/custom-profile security requirement: https://developer.chrome.com/blog/remote-debugging-port
- Win32 cross-process parenting and DPI caveats: https://learn.microsoft.com/windows/win32/api/winuser/nf-winuser-setparent
...
- Integrated-browser verification: the packaged Windows shell launched Chromium app-mode with the existing manually authenticated dedicated profile, verified its dynamic DevTools listener was loopback-only and owned by the app Chromium process tree, docked the live ChatGPT UI while keeping Tauri responsive, and completed real Chat Completions and Responses requests with exact expected text. Undock, redock, window resize, Stop, process-tree cleanup, CDP listener removal, API shutdown, and restart all passed. Manual login remains a separate no-CDP/no-Playwright flow; macOS/Linux retain the external Playwright-owned window.
