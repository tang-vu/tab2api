...
- On this checkout's host, WSL `gh`/`git` authenticate as a GitHub account without write access, while the Windows `gh.exe`/`git.exe` authenticate as the repository owner. From WSL, run git/gh operations through the Windows binaries (for example `git.exe -C <repo-root> push` and `gh.exe -R tang-vu/tab2api ...`), or `gh auth login` inside WSL with the owning account.
