...
try {
    ...
    if (($status -ne 302 -and $status -ne 303) -or -not $accessHost.EndsWith('.cloudflareaccess.com', [StringComparison]::OrdinalIgnoreCase)) {
        throw "Cloudflare Access is not protecting tab2api.tangvu.dev (probe HTTP $status). Create a deny-by-default Access application before installing the tunnel."
    }
}
finally {
...
Register-ScheduledTask -TaskName $taskName -Action $action -Trigger $trigger -Settings $settings -Principal $principal -Description 'Access-protected Cloudflare Tunnel for the loopback-only tab2api origin.' -Force | Out-Null
