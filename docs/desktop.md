...
The tunnel card includes an in-app, localized setup guide and can open the exact private runtime folder in File Explorer without returning its path to the WebView. Settings support English, Vietnamese, Simplified Chinese, Japanese, Korean, Spanish, French, and German. On first launch the WebView selects from the operating-system locale; it never performs IP geolocation or sends locale data over the network. Only the selected language code and optional public tunnel hostname are stored in the controller WebView's local settings storage. If that storage is unavailable, the app continues with local locale detection and does not fail startup.

The native controller is single-instance. A second launch exits after asking the existing instance to show, unminimize, and focus its main window. Sign-in launch is an explicit, default-off native setting: the operating system starts the controller with one exact internal flag, the main window remains hidden in the system tray, and the local API/browser remain stopped until the owner presses Start. Status and mutations cross the WebView boundary only as a boolean; plugin commands stay denied to the WebView, backend errors are reduced to typed content-free messages, and concurrent mutations fail fast. The source-tree Scheduled Task and installed desktop setting are alternative personal deployment modes and must not be enabled together.

The main window also has Browser, API Docs, and Keys & Usage tabs. The service card distinguishes a healthy local API from a verified ChatGPT session. It runs one session check after the service reaches healthy state and also provides an explicit retry button; it never polls the live ChatGPT UI in the background. Each check opens a temporary provider tab through `/readyz`, is bounded to 45 seconds, and localizes actionable states such as login required, security challenge, rate limit, UI change, and browser disconnection. API Docs is fully local, follows the selected language, and documents authentication, core OpenAI- and Anthropic-compatible routes, Claude Code shell setup, projects, media, administrator routes, examples, and current limitations. Its export button writes the canonical `docs/api.md` to a newly named `tab2api-api*.md` file in Downloads, never overwrites an existing file, and never inserts credentials. Keys & Usage lists only key metadata, creates and shows a revocable client key once, and can copy either the raw key or a complete current-process PowerShell setup for Claude Code. Both clipboard writes require an explicit click and the key text is cleared from the DOM when the dialog closes. The tab also requires confirmation before revocation, displays content-free per-key/per-endpoint counters, and requires confirmation before resetting those counters. The local service must be running for administration. On Windows, switching away from Browser hides the docked native Chromium child; switching back restores and resizes it. An explicitly undocked external browser remains independent of the tab selection.

Native authenticated access is deliberately narrow. For each operation Rust first opens a separate unauthenticated IPv4-loopback connection and requires the exact `/healthz` identity; only then does it read the administrator token from the private app runtime (or the explicit environment override), reject header-unsafe values, and send one hard-coded administrator route or `/readyz`. The two connections share a four-second administration deadline or 45-second readiness deadline, redirects are not followed, the complete response is capped at 2.5 MiB, and the request buffer and administrator token are discarded before response parsing. Error messages never include the token or response body. Returned key, usage, and readiness structures reject unknown fields, are bounded, and are validated again before crossing the Tauri command boundary. The identity probe prevents accidental disclosure to a wrong local service; it cannot authenticate against a malicious process running as the same operating-system user.
...
`desktop-install-smoke.yml` is a separate `workflow_dispatch` gate on a fresh Windows runner. It builds the unsigned NSIS preview, installs silently with shortcuts disabled into a unique directory under `RUNNER_TEMP`, verifies the installed manifest/SBOM and offline fake-adapter plus loopback lifecycle, then exercises hidden controller startup, single-instance rejection, and lock recovery after a forced process exit. Before silent uninstall it adds only an isolated per-user sign-in launch probe matching the native registration; uninstall must remove that value and installation files while retaining an app-local profile marker because profile deletion was never requested. The script refuses a pre-existing value and removes only the exact probe it created on failure. Every installer, uninstaller, startup, shutdown, and cleanup wait is bounded. The workflow has read-only repository permissions and uploads no installer or other artifact.
...
The installer workflow passes the installed `sidecar/` path explicitly to that same smoke script only after the strict manifest check. `smoke-desktop-installer.ps1` refuses an existing or out-of-root install directory, uses the documented NSIS silent `/S` and no-shortcut `/NS` modes, and keeps `/D` as the final unquoted installer argument. Silent uninstall intentionally uses the normal NSIS temporary-copy flow so `uninstall.exe` can remove itself; because its launcher may exit before the temporary child, the test separately waits up to five minutes for the validated installation directory to disappear. Desktop lifecycle and registry probes require an explicit switch used by the isolated workflow. Cleanup is limited to the validated runner-temporary directory, the exact sign-in probe, and a non-sensitive uniquely named profile-retention probe.
