...
JSON body: `prompt` is required; `model` is metadata; `n` must be `1`, `size` and `quality` must be `auto`, and `response_format` must be `b64_json`. The adapter requests one image through the public UI, waits for the generated image element, then screenshots that element as PNG. The response contains `created` and one `data[].b64_json`; `X-Tab2api-Image-Mode: ui-element-screenshot` discloses the extraction method. The screenshot reflects displayed UI resolution, not the original asset bytes.
