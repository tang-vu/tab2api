...
- `GET /admin/session`: reports the last session state the provider observed (`ready`, `login_required`, `security_challenge`, `generation_in_progress`, `rate_limited`, `ui_changed`, or `browser_disconnected`). It never opens a browser tab, so it is cheap to poll; before the first probe or turn it reports `browser_disconnected`. Use `GET /readyz` for a live probe.
