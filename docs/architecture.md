...
- `adapters/chatgpt/`: all URL/UI assumptions. `selector-contracts.ts` is the single registry of volatile locators (candidates, cardinality, text predicates, typed failure); `observe-dom.ts` is a self-contained observer that snapshots the whole page in one `page.evaluate`; `turn-lifecycle.ts` owns the phase machine and post-submit error boundary; `session.ts`, `composer.ts`, `turn.ts`, `image-turn.ts`, and `projects.ts` compose those into the turn flows.
...
- `observability/`: Pino configuration with allowlisted request serialization and redaction, plus a bounded operational `EventLog` and `MetricsRegistry` exposed at `/admin/diagnostics` and `/admin/metrics`.
...
Creating, listing, deleting, and uploading to a ChatGPT project are provider capabilities for the same reason. tab2api keeps no record of them: projects belong to the ChatGPT account, so listing reads live browser state and deletion acts on the identifier the client supplies. The projects grid publishes no identifier, so listing and deleting cost roughly one navigation per project and are bounded rather than proportional to the account.
...
5. The adapter opens a new page, navigates to the public ChatGPT root (a new conversation), classifies state, records the assistant-message baseline, fills the visible composer, and submits once. A project-scoped route navigates to that project instead, and a supplied conversation id continues that thread; both identifiers are validated against anchored, charset-restricted patterns before any navigation, in the API layer and again in the adapter. Every observation is one atomic `page.evaluate` running the self-contained DOM observer — session classification, turn ids, assistant state, pending markers, completion actions, and per-contract evidence in a single snapshot — so a poll can never mix two different renders, and the identical code runs offline against linkedom fixtures in the compatibility suite. Every turn advances an explicit lifecycle (`navigating → observing → preparing → submitting → submitted → generating → completing → done`); the transition table is enforced and the send gesture is a one-way boundary after which `postSubmit` stays true.
...
7. Text is mapped to `chatgpt-web` or the explicitly named Anthropic compatibility shape; generated image elements are temporarily rendered at their intrinsic dimensions and captured as bounded lossless PNG without reading or fetching their private URL, while STT uses a bounded audio attachment. Image generation may first upload bounded reference images through the same composer file input the chat routes use; because those uploads also appear in the transcript, such a turn locates the answer with assistant-scoped selectors only, so a reference is never captured in its place. The page closes in `finally` for success, error, timeout, and cancellation. TTS is a separate, labelled OS backend but still enters the bounded work queue for flood control.
...
The post-submit boundary is reflected in the error taxonomy rather than only in comments: a request-deadline abort after the send gesture is `generation_timeout` (the answer may still finish upstream), any other unexpected post-submit failure is `submission_uncertain`, and a bound turn that disappears mid-generation is `generation_interrupted`. Pre-submit failures keep their honest classes — `navigation_failed`, `composer_unavailable`, `attachment_failed`, `unsupported_capability`, `conversation_not_found`, `project_not_found` — because nothing was delivered and the caller may retry safely. Request cancellation and deadlines propagate through navigation, observation, composer interaction, uploads, and polling so a cancelled request does not wait out a full operation timeout.
