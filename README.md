...
- `GET /admin/metrics`, `GET /admin/diagnostics` (administrator only; content-free counters, per-error-code counts, selector-contract fingerprint, capability snapshot, unsatisfied contracts, bounded event log)
...
- Optional `temporary: true` (or `TAB2API_TEMPORARY_CHAT=true` as the default) runs each turn in a ChatGPT Temporary Chat so nothing is kept in account history; it cannot be combined with `conversation_id` or project routes, and the request fails `ui_changed` if the UI stops confirming the mode.
- Optional `reasoning_effort` (`minimal`–`xhigh`) drives the composer's effort control only; it never selects a different model and fails `ui_changed` when the account's picker lacks a matching option.
- Serialized prompts are preflight-checked against `TAB2API_MAX_PROMPT_TOKENS` (default 104,000 estimated `o200k` tokens) and rejected before a tab opens when oversized; the ceiling is measured product behaviour, not a per-account guarantee.
- Token counts are not visible in the UI. Chat Completions returns zero counts with `tab2api.usage_available=false`; Responses returns `usage: null`; Anthropic Messages returns zero usage and labels token counting as estimated. These values mean “unknown,” not zero actual usage. The `/v1/messages/count_tokens` estimate and the admin usage snapshot do use the real `o200k_base` tokenizer plus measured per-turn reserves, which is still an estimate, not billing data.
...
npm run status                        # service, session, and queue/drain state (--json for machine output)
